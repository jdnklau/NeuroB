...
/**
 * Reduced version of F275. Same as {@link BAst109Reduced} but also with cardinality.
 */
public class BAst110Features extends PredicateFeatures {
    ...
    public static class Generator implements PredicateFeatureGenerating<BAst110Features> {

        @Override
        public BAst110Features generate(BPredicate predicate, @Nullable MachineAccess machineAccess)
                throws FeatureCreationException {

            BAstFeatureData data = BAstFeatureCollector.collect(predicate, machineAccess);
            return new BAst110Features(predicate, generateArray(data));
