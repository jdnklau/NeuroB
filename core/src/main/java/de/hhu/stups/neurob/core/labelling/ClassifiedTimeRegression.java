 ...
 *     <li>t + timeout for unknown, error or timeout.</li>
 * </ul>
 *
 * <p>
 * If in the constructor no timeout values are set, the defaults are taken,
 * dictated by {@link ClassifiedTimeRegression#defaultTimeout}
 * and {@link ClassifiedTimeRegression#defaultTimeoutUnit}.
 */
public class ClassifiedTimeRegression extends PredicateLabelling {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(ClassifiedTimeRegression.class);
     ...
     *         created.
     */
    public ClassifiedTimeRegression(BPredicate predicate, MachineAccess bMachine, Backend... backends)
     ...
     *         created.
     */
    public ClassifiedTimeRegression(String predicate, MachineAccess bMachine, Backend... backends)
            throws LabelCreationException {
     ...
     *         created.
     */
    public ClassifiedTimeRegression(BPredicate predicate, Long timeOut, TimeUnit timeOutUnit,
        ...
        for (int i = 0; i < labellingDimension; i++) {
            labellingArray[i] = Double.valueOf(calcCost(timings.get(backends[i])));
        }
    }
     ...
     *         created.
     */
    public ClassifiedTimeRegression(String predicate,
     ...
     *         created.
     */
    public ClassifiedTimeRegression(BPredicate predicate, Map<Backend, TimedAnswer> timeMapping,
            Backend... backends) {
     ...
     *         created.
     */
    public ClassifiedTimeRegression(String predicate, Map<Backend, TimedAnswer> timeMapping, Backend... backends) {
    ...
    public ClassifiedTimeRegression(BPredicate predicate, Long timeOut, TimeUnit timeOutUnit,
        ...
        for (int i = 0; i < labellingDimension; i++) {
            if (timings.containsKey(backends[i])) {
                labellingArray[i] = new Double(calcCost(
     ...
     */
    public ClassifiedTimeRegression(BPredicate predicate,
     ...
     * @return
     */
    Long calcCost(TimedAnswer response) {
        return calcCost(response, timeOut, timeUnit);
    }

    static Long calcCost(TimedAnswer answer, Long timeOut, TimeUnit timeUnit) {
        if (answer == null) {
            return -1L; // TODO: -1 would serve as indicator something is missing. Keep it that way?
                        // NOTE: Same question arises in HealyTimings
        }
        ...
        if (Answer.VALID.equals(response) || Answer.INVALID.equals(response)
            ...
            return nanoseconds;
        } else if (Answer.UNKNOWN.equals(response) || Answer.ERROR.equals(response) || Answer.TIMEOUT.equals(response)) {
    ...
    static Double[] timingsToCostArray(TimedAnswer[] responses, Long timeOut, TimeUnit timeUnit) {
        return Arrays.stream(responses).map(r -> calcCost(r, timeOut, timeUnit))
                .map(Double::valueOf)
                .toArray(Double[]::new);
    }
    ...
    public static class Generator
            implements PredicateLabelGenerating<ClassifiedTimeRegression> {
        ...
        @Override
        public ClassifiedTimeRegression generate(BPredicate predicate, MachineAccess machineAccess)
                throws LabelCreationException {
            return new ClassifiedTimeRegression(predicate, timeout, timeoutUnit, machineAccess, backends);
        }
    }

    public static class Translator implements LabelTranslation<PredDbEntry, ClassifiedTimeRegression> {

        @Override
        public ClassifiedTimeRegression translate(PredDbEntry origLabels) {
            Backend[] backends = origLabels.getBackendsUsed();

            return new ClassifiedTimeRegression(origLabels.getPredicate(),
